...
- [ ] **初始化 Rust 项目**: 在 `server` 目录下初始化 Cargo 项目（或替换原 Python 目录）。
- [ ] **配置核心依赖**: 
  - 异步/Web: `tokio`, `axum`, `tokio-websockets` (或 `axum` 自带的 `ws`)
  - 输入控制: `enigo`
  - 服务发现: `mdns-sd` 或 `zeroconf` (Rust版)
  - 托盘图标: `tray-icon`, `tao` (用于主线程事件循环)
  - 静态资源: `rust-embed` (用于打包前端产物)
  - 字节解析: `bytes`, `byteorder`
- [ ] **构建脚本配置 (`build.rs`)**: 配置 Windows/macOS 的应用图标和应用清单 (Manifest)。
...
- [ ] **封装 `MouseController`**:
  - [ ] 实现相对移动 (Move)
  - [ ] 实现鼠标点击 (Click - Left/Right/Middle)
  - [ ] 实现鼠标拖拽按下与释放 (Drag Start/End)
  - [ ] 实现滚轮滚动 (Scroll - 垂直/水平)
- [ ] **封装 `KeyboardController`**:
  - [ ] 实现纯文本输入 (处理 UTF-8，考虑 Enigo 的 `text` 方法或剪贴板 fallback)
  - [ ] 实现功能键映射 (映射前端传来的 `"enter"`, `"backspace"`, `"esc"` 等至 Enigo 的 `Key` 枚举)
  - [ ] 实现修饰键 (Modifier Keys) 状态管理，解析位掩码 (Bitmask: Ctrl/Shift/Alt/Win) 并包裹执行目标按键/点击。
- [ ] **系统权限处理 (macOS)**: 编写文档或提示，引导用户在 macOS 开启辅助功能权限；处理没有权限时的容错。

## 阶段 3: 二进制协议解析层 (Protocol Parser)
> **参考 `v3_1.md` - `v3_3.md`**, 保持大端序 (Big-Endian) 解析。

- [ ] **定义指令结构体 (Enums/Structs)**: 将接收到的二进制流映射为强类型的 Rust 枚举。
- [ ] **解析 `Move (0x01)`**: `[OpCode: 1B] [DeltaX: Int16] [DeltaY: Int16]`
- [ ] **解析 `Click (0x02)`**: `[OpCode: 1B] [Button: 1B] [ModifierMask: 1B]` (V3.3 新增掩码)
- [ ] **解析 `Scroll (0x03)`**: `[OpCode: 1B] [ScrollX: Int16] [ScrollY: Int16]`
- [ ] **解析 `Drag (0x04)`**: `[OpCode: 1B] [State: 1B]`
- [ ] **解析 `Text (0x05)`**: `[OpCode: 1B] [UTF-8 Bytes...]`
- [ ] **解析 `KeyAction (0x06)`**: `[OpCode: 1B] [ModifierMask: 1B] [KeyName: UTF-8 Bytes...]`
